...
FROM project p
LEFT JOIN experiment e ON e.project_id = p.id
LEFT JOIN job j ON j.experiment_id = e.id
LEFT JOIN job_context jc ON jc.job_id = j.id
LEFT JOIN act_hi_actinst ai ON ai.act_id_ = jc.task AND ai.proc_inst_id_ = (SELECT pi.id_ FROM act_hi_procinst pi WHERE pi.business_key_ = CAST(j.id AS VARCHAR))
